...
import action.BookViewForCustomerAction;
...
public class BookListFrontController extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		//도서 상세보기 페이지
			...
			action=new  BookViewForCustomerAction();
			try {
				forward=action.execute(req, resp);
			}catch(Exception e) {
				e.printStackTrace();
		...
		//리뷰 등록하기
		else if(command.equals("/reviewRegistForm.book")) {
			forward=new ActionForward();
			forward.setPath("review/qna_review_regist.jsp");
		}
