...
import action.CusInfoModifyAction;
import action.CusInfoModifyFormAction;
...
public class LoginFrontController extends HttpServlet{
	...
	public void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		//회원정보 수정 폼
		else if(command.equals("/cusInfoModifyForm.lo")) {
			action=new CusInfoModifyFormAction();
			try {
				forward=action.execute(req, resp);
			}catch(Exception e){
				e.printStackTrace();
		...
		//회원정보 수정 (update)
		else if(command.equals("/cusInfoModify.lo")) {
			action=new CusInfoModifyAction();
			try {
				forward=action.execute(req, resp);
			}catch(Exception e){
				e.printStackTrace();
